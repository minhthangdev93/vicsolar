	...
	<div class="section-content relative">
		<?php if ( $show_shared_header ) : ?>
			<section style="font-family: Montserrat,Inter,Segoe UI,Roboto,Arial,sans-serif; width: 100%; padding: 18px 16px 12px; margin: 0; box-sizing: border-box; background: transparent; text-align: center;">
				...
				<?php if ( $pricing_sub ) : ?>
					<div style="margin: 0; font-size: clamp(13px,1.25vw,17px); font-weight: 550; line-height: 1.55; color: rgba(11,19,32,.82);"><?php echo electro_child_landing_kses_content( $pricing_sub ); ?></div>
				<?php endif; ?>
			</section>
		<?php endif; ?>
		...
		<section style="font-family: Montserrat,Inter,Segoe UI,Roboto,Arial,sans-serif; padding: 0; margin: 0; box-sizing: border-box; background: transparent;">
			<div style="width: 100%; background: transparent; padding: 10px 16px; margin: 0; box-sizing: border-box; text-align: center;">
				<?php if ( $show_shared_header ) : ?>
					<?php if ( $tier_title ) : ?>
						<h3 style="margin: 0; padding: 0; font-size: 28px; line-height: 1.08; font-weight: 950; color: #f05a12; letter-spacing: .2px; text-transform: uppercase;"><?php echo electro_child_landing_kses_content( $tier_title ); ?></h3>
					<?php endif; ?>
					<?php if ( $tier_sub ) : ?>
						<div style="margin-top: 6px; font-size: 14px; line-height: 1.35; font-weight: 800; color: rgba(240,90,18,.92);"><?php echo electro_child_landing_kses_content( $tier_sub ); ?></div>
					<?php endif; ?>
				<?php else : ?>
					<?php if ( $tier_title ) : ?>
						<div style="margin: 0; font-size: clamp(13px,1.25vw,17px); font-weight: 550; line-height: 1.55; color: rgba(11,19,32,.82);">
							<span style="color: #f05a12; font-size: 28px; font-weight: 950; letter-spacing: 0.2px; text-transform: uppercase; background-color: transparent;"><?php echo electro_child_landing_kses_content( $tier_title ); ?></span>
						</div>
					<?php endif; ?>
					<?php if ( $tier_sub ) : ?>
						<div style="margin: 0; font-size: clamp(13px,1.25vw,17px); font-weight: 550; line-height: 1.55; color: rgba(11,19,32,.82);">
							<span style="color: rgba(240, 90, 18, 0.92); font-size: 14px; font-weight: 800; background-color: transparent;"><?php echo electro_child_landing_kses_content( $tier_sub ); ?></span>
						</div>
					<?php endif; ?>
				<?php endif; ?>
			</div>
